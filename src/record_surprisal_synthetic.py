...
######################################
BOW_SYMBOL = "Ġ"  # This works fine and tested for GPT-2, LLama3.1, Llama3.2, and Qwen2.5.
######################################
# Helper functions
...
def parse_args():
    ap = argparse.ArgumentParser(description="Measuring word-level surprisals of metaphoric words in synthetic dataset.")
    
    ap.add_argument("--input", required=True, help="JSON with at least 'sentence' and 'offsets' fields.")
...
def main():
    ...
    for i, sent in tqdm(enumerate(sentences), total=len(sentences), desc="Processing sentences"):
        ...
        subtokens_span = subtoken_indices_for_char_span(offset_mapping, offset[0], offset[1])
        subtokens_span = torch.tensor(subtokens_span, device=device, dtype=torch.long)

        surp_sub = surprisal_tok[0, subtokens_span]
        surprisals_buggy_per_offset = surp_sub.detach().cpu().numpy().sum().item()
        if args.pimentel_fix:
            ...
            surprisals_fixed_per_offset = surp_fixed_sub.detach().cpu().numpy().sum().item()

        subtokens_ids = [labels[0][idx].item() for idx in subtokens_span]
        subtokens_str = tokenizer.decode(subtokens_ids)

        df.at[i, 'subtoken_ids'] = subtokens_ids
        df.at[i, 'subtoken_strs'] = subtokens_str
    ...
    if args.cloze:
        ...
        for i, sent in tqdm(enumerate(sentences), total=len(sentences), desc="Processing sentences"):
            ...
            surp_sub = surprisal_tok[0, subtokens_span]
            surprisals_buggy_j = surp_sub.detach().cpu().numpy().sum().item()
            if args.pimentel_fix:
                bow_sub  = bow_fix_tok[0, subtokens_span]
                bos_sub  = bos_fix_tok[0, subtokens_span]
                eow_sub  = eow_fix_tok[0, subtokens_span]
                ...
                surprisals_fixed_j = surp_fixed_sub.detach().cpu().numpy().sum().item()
            
            subtokens_ids_j = [labels[0][idx].item() for idx in subtokens_span]
            subtokens_str_j = tokenizer.decode(subtokens_ids_j)

            df.at[i, 'subtoken_ids_cloze'] = subtokens_ids_j
            df.at[i, 'subtoken_strs_cloze'] = subtokens_str_j
            df.at[i, 'surprisal_buggy_cloze'] = surprisals_buggy_j
            if args.pimentel_fix:
                df.at[i, 'surprisal_fixed_cloze'] = surprisals_fixed_j

    #####################################
    #####################################
    #####################################
    save_name = f"{args.input.split('/')[-1].split('.')[0]}_{model_id.replace('/', '_')}{'_cloze' if args.cloze else ''}{'_pimentel' if args.pimentel_fix else ''}{'_' + args.model_revision if args.model_revision else ''}.parquet"
